...
class KNN:
    ...
    def compute_distances_no_loops(self, X):
        ...
        dists = np.zeros((num_test, num_train), np.float32)
        dists = np.abs(X[:, None] - self.train_X).sum(axis=2)
        return dists
    ...
    def predict_labels_binary(self, dists):
        ...
        for i in range(num_test):
            closest_indices = np.argsort(dists[i])[:self.k]
            closest_labels = self.train_y[closest_indices]
            pred[i] = np.bincount(closest_labels).argmax()
        return pred
    ...
    def predict_labels_multiclass(self, dists):
        ...
        for i in range(num_test):
            ...
            closest_labels = self.train_y[closest_idxs]
            pred[i] = np.bincount(closest_labels).argmax()
        return pred
